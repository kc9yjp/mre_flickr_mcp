...
// forked from design-sync lib/source-kit.mjs — this app's src/ has a real
// entrypoint (main.tsx) with top-level DOM side effects
// (document.getElementById('root').render(...)), unlike a component
// library's src/. Sweeping it into the synthetic `export *` entry throws at
// bundle-evaluation time and takes down every export in the bundle, not just
// App's (hence [BUNDLE_EXPORT] flagging all 21 components as missing). Excluded
// alongside stories/test files.
const NON_IMPL_RX = /\.(stories|test|spec)\.|(^|\/)main\.tsx$/;
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // forked: `export * from <file>` never re-exports a *default* export
    // (ES module semantics — `export *` only forwards named bindings), so a
    // component declared `export default function Name()` (this repo's App)
    // would silently vanish from the bundle's window.<Namespace> even though
    // deriveComponentsFromSrc() below still lists it by name. Also emit an
    // explicit `export { default as Name }` line for any file whose default
    // export has a recoverable declared name.
    ...
    for (const p of comps) {
      ...
      const sf = defaultProject.addSourceFileAtPathIfExists(p);
      const defaultDecls = sf?.getExportedDeclarations().get('default');
      const name = defaultDecls?.map((d) => d.getName?.()).find((n) => n && n !== 'default');
