...
CREATE TABLE IF NOT EXISTS groups (
    ...
    description TEXT, user_note TEXT, needs_summary INTEGER DEFAULT 1,
    summary_md TEXT, is_milestone INTEGER DEFAULT 0,
    fave_min INTEGER, view_min INTEGER, open_subject INTEGER,
    ai_keywords TEXT, summary_generated_at INTEGER
);
...
def mem_db(tmp_path) -> str:
    ...
    conn.execute(
        ...
        ("group1@N00", "Landscape Lovers", 500, 1000, now, "Landscape photography",
         "", 0, "A group for landscape photography.", 0, None, None, None,
         "landscape nature scenery", now),
    )
    conn.execute(
