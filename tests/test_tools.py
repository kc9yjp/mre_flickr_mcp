...
@pytest.fixture()
def db(mem_db):
    """Patch flickr_mcp.db() to create fresh connections to the test DB file each call.
    Yields an open connection for test assertions; handlers can close() their own copies freely."""
    ...
    with patch("flickr_mcp.db", side_effect=_make_conn):
        yield verify_conn
    verify_conn.close()
...
@pytest.fixture()
def api_get():
    with patch("flickr_mcp._api_get") as m:
        yield m


@pytest.fixture()
def api_post():
    with patch("flickr_mcp._api_post") as m:
        yield m


@pytest.fixture()
def creds():
    with patch("flickr_mcp._load_credentials", return_value=FAKE_CREDS):
        yield FAKE_CREDS
...
class TestSearchPhotos:
    ...
    async def test_returns_all_photos_by_default(self, db):
        import flickr_mcp
        result = await flickr_mcp._search_photos({})
        photos = _json(result)
        assert len(photos) == 1
    ...
    async def test_keyword_filter(self, db):
        import flickr_mcp
        result = await flickr_mcp._search_photos({"query": "Test"})
        assert len(_json(result)) == 1

        result = await flickr_mcp._search_photos({"query": "nope"})
        assert len(_json(result)) == 0

    @pytest.mark.asyncio
    async def test_tag_filter(self, db):
        import flickr_mcp
        result = await flickr_mcp._search_photos({"tags": "sunset"})
        assert len(_json(result)) == 1

        result = await flickr_mcp._search_photos({"tags": "portrait"})
        assert len(_json(result)) == 0

    @pytest.mark.asyncio
    async def test_date_range_filter(self, db):
        import flickr_mcp
        result = await flickr_mcp._search_photos({"date_from": "2024-01-01", "date_to": "2024-12-31"})
        assert len(_json(result)) == 1

        result = await flickr_mcp._search_photos({"date_from": "2025-01-01"})
        assert len(_json(result)) == 0
    ...
    async def test_incomplete_filter(self, db):
        ...
        result = await flickr_mcp._search_photos({"incomplete": True})
        assert len(_json(result)) == 0
...
class TestGetPhoto:
    ...
    async def test_found(self, db):
        import flickr_mcp
        result = await flickr_mcp._get_photo({"id": "photo1"})
        photo = _json(result)
    ...
    async def test_not_found(self, db):
        import flickr_mcp
        result = await flickr_mcp._get_photo({"id": "missing"})
...
class TestGetSummary:
    ...
    async def test_summary_shape(self, db):
        import flickr_mcp
        result = await flickr_mcp._get_summary()
...
class TestUpdatePhoto:
    ...
    async def test_update_title_and_tags(self, db, api_post):
        ...
        result = await flickr_mcp._update_photo({"id": "photo1", "title": "New Title", "tags": "rain"})
    ...
    async def test_update_tags_only(self, db, api_post):
        import flickr_mcp
        api_post.return_value = {"stat": "ok"}
        result = await flickr_mcp._update_photo({"id": "photo1", "tags": "fog"})
        assert "tags" in _text(result)
...
class TestAlbums:
    ...
    async def test_find_albums(self, db):
        import flickr_mcp
        result = await flickr_mcp._find_albums({"query": "My"})
        albums = _json(result)
        assert len(albums) == 1
    ...
    async def test_find_albums_no_match(self, db):
        import flickr_mcp
        result = await flickr_mcp._find_albums({"query": "xyz"})
    ...
    async def test_get_album_photos(self, db, creds, api_get):
        ...
        result = await flickr_mcp._get_album_photos({"album_id": "album1"})
        data = _json(result)
        assert data["total"] == 1
...
class TestGroups:
    ...
    async def test_find_groups_by_name(self, db):
        import flickr_mcp
        result = await flickr_mcp._find_groups({"query": "Landscape"})
        groups = _json(result)
        assert len(groups) == 1
    ...
    async def test_find_groups_by_keyword(self, db):
        import flickr_mcp
        result = await flickr_mcp._find_groups({"query": "nature"})
        assert len(_json(result)) == 1

    @pytest.mark.asyncio
    async def test_find_groups_no_match(self, db):
        import flickr_mcp
        result = await flickr_mcp._find_groups({"query": "zzz"})
        assert "No groups" in _text(result)

    @pytest.mark.asyncio
    async def test_set_group_keywords(self, db):
        import flickr_mcp
        result = await flickr_mcp._set_group_keywords({"group_id": "group1@N00", "keywords": "mountains snow"})
    ...
    async def test_set_group_keywords_not_found(self, db):
        import flickr_mcp
        result = await flickr_mcp._set_group_keywords({"group_id": "bad@N00", "keywords": "x"})
        assert "not found" in _text(result)
...
class TestContacts:
    ...
    async def test_protect_contact(self, db):
        import flickr_mcp
        result = await flickr_mcp._protect_contact({"contact_id": "contact1@N00", "reason": "friend"})
    ...
    async def test_find_unfollow_candidates_excludes_protected(self, db):
        import flickr_mcp
        await flickr_mcp._protect_contact({"contact_id": "contact1@N00"})
        result = await flickr_mcp._find_unfollow_candidates({})
        text = _text(result)
        # Protected contact should not appear — either empty message or empty list
...
class TestSetVisibility:
    ...
    async def test_make_private(self, db, api_post):
        import flickr_mcp
        api_post.return_value = {"stat": "ok"}
        result = await flickr_mcp._set_visibility({"id": "photo1", "is_public": False})
    ...
    async def test_make_public(self, db, api_post):
        import flickr_mcp
        api_post.return_value = {"stat": "ok"}
        result = await flickr_mcp._set_visibility({"id": "photo1", "is_public": True})
...
class TestLocation:
    ...
    async def test_set_location(self, api_post):
        import flickr_mcp
        api_post.return_value = {"stat": "ok"}
        result = await flickr_mcp._set_location({"id": "photo1", "lat": 37.77, "lon": -122.41})
    ...
    async def test_remove_location(self, api_post):
        import flickr_mcp
        api_post.return_value = {"stat": "ok"}
        result = await flickr_mcp._remove_location({"id": "photo1"})
...
class TestComments:
    ...
    async def test_add_comment(self, api_post):
        ...
        result = await flickr_mcp._add_comment({"photo_id": "photo1", "comment_text": "Great!"})
    ...
    async def test_delete_comment(self, api_post):
        import flickr_mcp
        api_post.return_value = {"stat": "ok"}
        result = await flickr_mcp._delete_comment({"comment_id": "cmt1"})
    ...
    async def test_get_photo_comments(self, api_get):
        ...
        result = await flickr_mcp._get_photo_comments({"photo_id": "photo1"})
...
class TestFindWeakPhotos:
    ...
    async def test_returns_photo(self, db):
        import flickr_mcp
        result = await flickr_mcp._find_weak_photos({"min_age_days": 0})
        photos = _json(result)
    ...
    async def test_require_zero_favorites_excludes_faved(self, db):
        ...
        result = await flickr_mcp._find_weak_photos({"min_age_days": 0, "require_zero_favorites": True})
        photos = _json(result)
